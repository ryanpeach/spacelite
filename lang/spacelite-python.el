...
(defun spacelite/init-python ()
  ...
  (use-package python
    ...
    ;; Running / Compiling
    (spacelite/declare-prefix-for-mode 'python-mode "c" "compile" "Python compilation functions")
    (spacelite/set-leader-keys-for-major-mode 'python-mode "cm" 'python-mypy-run)
    (spacelite/set-leader-keys-for-major-mode 'python-mode "cc" 'python-run)
    ...
    (use-package
      ...
      (spacelite/set-leader-keys-for-major-mode 'python-mode "ct" 'pytest)

      ;; Debugging
      (spacelite/set-prefix-for-mode 'python-mode "d" "debug" "Python debug commands")
      (spacelite/set-leader-keys-for-major-mode 'python-mode "db" 'elpy-pdb-toggle-breakpoint-at-point)
      (spacelite/set-leader-keys-for-major-mode 'python-mode "dd" 'elpy-pdb-debug-buffer)
      (spacelite/set-leader-keys-for-major-mode 'python-mode "dt" 'pytest)

      ;; Formatting
      (spacelite/set-prefix-for-mode 'python-mode "f" "file")
      (spacelite/set-leader-keys-for-major-mode 'python-mode "ff" 'elpy-format-code)

      ;; Profiling
      (spacelite/set-leader-keys-for-major-mode 'python-mode "p" 'elpy-profile-buffer-or-region)

      ;; Refactoring
      (spacelite/set-prefix-for-mode 'python-mode "r" "refactor" "Refactor Python")

      ;; Navigation
      (defalias 'elpy-goto-return 'pop-tag-mark)
      (spacelite/set-prefix-for-mode 'python-mode "." "navigation" "move around to definitions")
      (spacelite/set-leader-keys-for-major-mode 'python-mode ".^" 'elpy-set-project-root)
